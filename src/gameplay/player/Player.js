...
import { activeMultiplier, applyPowerUp } from './PowerUps.js';
import { ITEMS } from '../../data/items.js';

const BASE_STATS = { maxHp: 100, meleeDamage: 22, projectileDamage: 14, sprintSpeed: PHYSICS.sprintSpeed, powerupDuration: 1 };

export class Player {
  constructor(x, y, bus, projectiles, particles, skills, inventory) {
    Object.assign(this, { x, y, w: 38, h: 58, vx: 0, vy: 0, facing: 1, grounded: false, wall: 0, hp: 100, maxHp: 100, energy: 100, maxEnergy: 100, xp: 0, level: 1, gold: 0, keys: 0, crystals: 0, jumps: 0, invulnerable: 0, attackTimer: 0, rangedTimer: 0, powerups: [], bus, projectiles, particles, skills, inventory, spawn: { x, y } });
  ...
  applySkillEffects() {
    ...
    const effects = [...(this.skills?.unlocked || []).map(id => SKILLS.find(skill => skill.id === id)?.effect), ...Object.values(this.inventory?.equipment || {}).map(id => ITEMS[id]?.effect)];
  ...
  useConsumable(item) { applyPowerUp(this, item); }
  activeDamage(damage) { return damage * activeMultiplier(this, 'damageBoost'); }
